...
public class LinkedInController {
    ...
    /**
     * Check if LinkedIn is connected and get authorization URL if needed
     * @param authentication The authentication object containing user details
     * @return Connection status and authorization URL if not connected
     */
    /**
     * Get all connected LinkedIn accounts (Admin only)
     */
    @GetMapping("/accounts")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<Map<String, Object>>> getAllLinkedInAccounts() {
        try {
            List<LinkedInUser> accounts = linkedInUserRepository.findAll();
            List<Map<String, Object>> result = accounts.stream()
                .map(account -> {
                    ...
                    accountInfo.put("userId", account.getUser().getId());
                    accountInfo.put("connectedAt", account.getCreatedAt());
                    return accountInfo;
                })
            ...
            log.error("Error fetching LinkedIn accounts: {}", e.getMessage(), e);
    ...
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<Map<String, Object>> checkConnection(Authentication authentication) {
        try {
            String username = authentication.getName();
            ...
            boolean isConnected = linkedInService.isLinkedInConnected(user.getId());
            response.put("connected", isConnected);
            
            if (!isConnected) {
                // Get the authorization URL for the user to connect their LinkedIn account
                String authUrl = linkedInService.getAuthorizationUrl(user.getId());
                if (authUrl != null) {
                    response.put("authorizationUrl", authUrl);
                    response.put("message", "Please connect your LinkedIn account to continue");
                } else {
                    response.put("error", "Failed to generate LinkedIn authorization URL");
                }
            } else {
                Optional<LinkedInUser> linkedInUser = linkedInUserRepository.findFirstByUser(user);
                linkedInUser.ifPresent(lu -> {
                    response.put("name", lu.getName());
                    response.put("email", lu.getEmail());
                    response.put("connectedAt", lu.getCreatedAt());
                });
            }
            ...
            log.error("Error checking LinkedIn connection status: {}", e.getMessage(), e);
            return ResponseEntity.status(500)
                .body(createErrorResponse("Error checking LinkedIn connection: " + e.getMessage()));
        }
    }
     ...
     * @param linkedInAccountId (Optional) ID of the LinkedIn account to use for publishing.
     *                         If not provided, will use the current user's LinkedIn account.
     *                         Only users with ROLE_ADMIN can specify a different LinkedIn account.
     * @param authentication The authentication object containing user details
     * @return Response with the result of the publishing operation
     */
    @PostMapping("/publish-post/{postId}")
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity<Map<String, Object>> publishPost(
            @PathVariable Long postId,
            @RequestParam(required = false) Long linkedInAccountId,
            Authentication authentication) {
        ...
        // Find the LinkedIn account to use
        Optional<LinkedInUser> linkedInUserOpt;
        
        if (linkedInAccountId != null) {
            // Verify user has permission to use the specified LinkedIn account
            if (!authentication.getAuthorities().stream()
                    .anyMatch(grantedAuthority -> grantedAuthority.getAuthority().equals("ROLE_ADMIN"))) {
                return ResponseEntity.status(403).body(Map.of(
                        "success", false,
                        "error", "Only administrators can specify a LinkedIn account to use"
                ));
            }
            
            // Find the specified LinkedIn account
            linkedInUserOpt = linkedInUserRepository.findById(linkedInAccountId);
            if (linkedInUserOpt.isEmpty()) {
                return ResponseEntity.status(404).body(Map.of(
                        "success", false,
                        "error", "Specified LinkedIn account not found"
                ));
            }
        } else {
            // Use the current user's LinkedIn account
            linkedInUserOpt = linkedInUserRepository.findFirstByUser(currentUser);
        }
        ...
        String accessToken = linkedInUser.getAccessToken();
        String personUrn = linkedInUser.getPersonUrn();

        if (accessToken == null || personUrn == null) {
            return ResponseEntity.status(400).body(Map.of(
                    "success", false,
                    "error", "LinkedIn account is not properly configured"
            ));
        }
        ...
        try {
            ...
            response.put("message", success ? "Post published successfully!" : "Failed to publish post");
            response.put("postId", postId);
            response.put("linkedInAccount", Map.of(
                "id", linkedInUser.getId(),
                "name", linkedInUser.getName(),
                "email", linkedInUser.getEmail(),
                "userId", linkedInUser.getUser().getId()
            ));
